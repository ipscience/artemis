...
| **地理分析** | 国/地域別コロプレス地図・バブルマップ |
| **エクスポート** | CSV/JSON/HTML/Excelレポート出力 |
...
- **SheetJS (xlsx.js) 0.18.5** - Excelエクスポート
- **Transformers.js** - ブラウザでのSBERT埋め込み (all-MiniLM-L6-v2)

### その他
- **Service Worker** - CDNアセットのキャッシュ（2回目以降の起動を高速化）
...
- **レスポンシブ**: デスクトップ・タブレット・**モバイル**対応（≤600pxでハンバーガーメニュー）
- **列設定の記憶**: 表示列のカスタマイズをLocalStorageに自動保存
- **言語切替**: ヘッダーのEN/JAリンクで切替可能
